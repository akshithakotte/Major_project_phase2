...
def train_cnn(X_train, X_test, y_train, y_test):
    ...
    return model, acc, prec, rec, f1, cm.tolist()
...
def train_random_forest(X_train, X_test, y_train, y_test):
    ...
    return rf_model, acc, prec, rec, f1, cm.tolist()


def save_metrics(cnn_metrics, rf_metrics, cnn_cm=None, rf_cm=None):
    ...
    if cnn_cm is not None:
        metrics["CNN"]["confusion_matrix"] = cnn_cm
    if rf_cm is not None:
        metrics["RandomForest"]["confusion_matrix"] = rf_cm
...
def train_all(csv_path: str = DATASET_PATH):
    """
    ...
    Returns raw (0-1) metric tuples for CNN and RF.
    """
    print("[INFO] Running full training pipeline...")

    X_train, X_test, y_train, y_test = load_and_preprocess(
        csv_path, training=True
    )

    cnn_model, cnn_acc, cnn_prec, cnn_rec, cnn_f1, cnn_cm = train_cnn(
        X_train, X_test, y_train, y_test
    )

    rf_model, rf_acc, rf_prec, rf_rec, rf_f1, rf_cm = train_random_forest(
        X_train, X_test, y_train, y_test
    )
